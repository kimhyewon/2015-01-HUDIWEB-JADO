...
CREATE TABLE IF NOT EXISTS `jado_dev`.`CART` (
  ...
  PRIMARY KEY (`SHOP_URL`, `CUSTOMER_ID`, `PRODUCT_ID`, `CART_TIME`),
...
CREATE TABLE IF NOT EXISTS `jado_dev`.`PAYMENT` (
  ...
  PRIMARY KEY (`SHOP_URL`, `CUSTOMER_ID`, `PRODUCT_ID`, `CART_TIME`, `PAY_TIME`),
  INDEX `fk_PAYMENT_CART1_idx` (`SHOP_URL` ASC, `CUSTOMER_ID` ASC, `PRODUCT_ID` ASC, `CART_TIME` ASC),
  CONSTRAINT `fk_PAYMENT_CART1`
    FOREIGN KEY (`SHOP_URL` , `CUSTOMER_ID` , `PRODUCT_ID` , `CART_TIME`)
    REFERENCES `jado_dev`.`CART` (`SHOP_URL` , `CUSTOMER_ID` , `PRODUCT_ID` , `CART_TIME`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
